...
| ------------- | ------------- | ----- |
| Components vs Nodes | In Unity, each `GameObject` can have multiple Components (via MonoBehaviour). In Godot, each Node is the component. | Create a separate node for each behavior, rather than a separate component. This requires rethinking logic around finding the target for a particular behavior script. Instead of using `gameObject` directly, in Godot you might operate on the parent node, for example. |
